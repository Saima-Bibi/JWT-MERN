...
export default function Email() {
 ...
 const res = await axios.post(`api/user/forget-Password?email=${value.email}`)

 if(res.data){
  localStorage.setItem('UserEmail', JSON.stringify(value.email))
  ...
  return (
    ...
    <form  onSubmit={handleSubmit}   className='border border-black rounded-md px-4 py-6 bg-white  '>
        ...
        <h2 className='text-slate-900 font-semibold mb-1'>Forget Password</h2>
<div className='mb-2'>
...
<Input
          ...
          <button className='h-9 w-[100%] bg-green-500 hover:bg-green-700  text-white font-semibold rounded-md'>Send otp</button>
          {/* <h5 className='mt-1 text-center'><span className=' underline underline-offset-2 text-green-500 hover:text-green-700  font-medium cursor-pointer text-sm' onClick={handleClick}>Resend Otp?</span></h5> */}
